...
double InsertionAVL(int a[], Node *root);
double deletionAVL(int a[], Node *root);


int main(){
    int *set = new int[MAX];
    int *del = new int[MAX/10];
    Node *root = NULL;
    ...
    /*AVL Tree Insert Experiment*/
    for (int run = 0; run < 10; run++){
        //shuffle sets
        random_shuffle(set, set+(MAX-1));
        random_shuffle(del, del+(MAX/10-1));
...
/*AVL tree insertion time Experiment Subroutine*/
double InsertionAVL(int a[], Node *root){
    int i = 0;
    clock_t startTime = clock();
    // CODE
    ...
    //compute execution duration in runtime
    return double( clock() - startTime ) / (double)CLOCKS_PER_SEC;
}

/*AVL tree deletion time Experiment Subroutine*/
double deletionAVL(int a[], Node *root){
    int i = 0;
    clock_t startTime = clock();
    // CODE
